...
public class Clustering_C {

    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.err.println("Usage: Clustering_C <input path> <output path> <max iterations> <convergence threshold>");
            System.exit(-1);
        }
        ...
        for (iteration = 0; iteration < maxIterations && !converged; iteration++) {
            ...
            job.setJarByClass(Clustering_C.class);
            job.setMapperClass(Clustering_C_Mapper.class);
            job.setReducerClass(Clustering_C_Reducer.class);
