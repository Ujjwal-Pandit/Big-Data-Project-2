...
public class Medoids_D {

    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.err.println("Usage: Medoids_D <input path> <output path> <max iterations> <convergence threshold>");
            System.exit(-1);
        }
        ...
        for (iteration = 0; iteration < maxIterations && !converged; iteration++) {
            ...
            job.setJarByClass(Medoids_D.class);
            job.setMapperClass(Medoids_D_Mapper.class);
            job.setCombinerClass(Medoids_D_Combiner.class);
            job.setReducerClass(Medoids_D_Reducer.class);
