...
public class Clustering_D {

    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.err.println("Usage: Clustering_D <input path> <output path> <max iterations> <convergence threshold>");
            System.exit(-1);
        }
        ...
        for (iteration = 0; iteration < maxIterations && !converged; iteration++) {
            ...
            job.setJarByClass(Clustering_D.class);
            job.setMapperClass(Clustering_D_Mapper.class);
            job.setCombinerClass(Clustering_D_Combiner.class);  // Add this line
            job.setReducerClass(Clustering_D_Reducer.class);
