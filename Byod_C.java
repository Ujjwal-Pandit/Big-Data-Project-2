...
public class Byod_C {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.err.println("Usage: Byod_C <input path> <output path> <max iterations> <convergence threshold>");
            System.exit(-1);
        }
        ...
        for (iteration = 0; iteration < maxIterations && !converged; iteration++) {
            ...
            job.setJarByClass(Byod_C.class);
            job.setMapperClass(Byod_C_Mapper.class);
            job.setReducerClass(Byod_C_Reducer.class);
